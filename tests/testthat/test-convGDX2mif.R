...
test_that("Test if REMIND reporting is produced as it should and check data integrity", {
  ...
  for (gdxPath in gdxPaths) {
    ...
    computedVariables <- unique(c(computedVariables, getItems(mifContent, dim = 3.3)))
  ...
  # check if all the variables used in project-specific mapping templates are
  # generated by the reporting
  templateVariables <- piamInterfaces::getREMINDTemplateVariables()
  missingVariables <- setdiff(templateVariables, computedVariables)

  # expect_true(all(computedVariables %in% templateVariables))

  # TODO: For now, this is a mere message. Make the test once the setup has
  # been completed.
