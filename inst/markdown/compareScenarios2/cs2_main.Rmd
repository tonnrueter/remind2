...
# TODO: Should be filtered in historical.mif or kept in plots
...
# TODO: Should be done in historical.mif?
# preprocess IEA WEO 2021
dataHist %>% 
  filter(str_detect(model, "^IEA WEO 2021")) %>% 
  select(model, region) %>% 
  distinct() ->
  ieaWeo
ieaWeo %>% 
  filter(
    !((str_ends(model, "GLO") & region == "World") |
    (str_ends(model, "regional") & region != "World"))) ->
  iaeWeoInvalid
dataHist %>% 
  anti_join(iaeWeoInvalid) %>% 
  mutate(model = str_remove(model, " GLO$| regional$")) ->
  dataHist
...
# TODO: Should probably be filtered in historical.mif
