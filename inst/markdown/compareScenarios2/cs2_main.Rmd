...
tibble(path = unname(params$mifScen)) %>% 
  ...
  mutate(data = map(path, read.quitte, factors=FALSE)) %>% 
  unnest(data) %>% 
  nest(-c(fileid, path, scenario)) ->
  dataScenNested
...
# Add suffixes "_1", "_2", ... in case of duplicated scenario names.
# Side note: In the unlikly case, one has scenario names
#   scenA, scenA, scenA_1, scenB,
# they will be renamed to
#   scenA_1, scenA_2, scenA_1, scenB
# in the first iteration of the following loop. After the second iteration we get
#   scenA_1_1, scenA_2, scenA_1_2, scenB.
while(anyDuplicated(dataScenNested$newScenarioName)) {
  warning("There are dupicated scenario names. They will be renamed.")
  dataScenNested %>% 
    group_by(newScenarioName) %>% 
    mutate(newScenarioName = 
      if (n() == 1) newScenarioName else paste(newScenarioName, 1:n(), sep="_")
    ) %>% 
    ungroup() -> 
    dataScenNested
}
