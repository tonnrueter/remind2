...
```{r read mifs, include=FALSE}
# TODO: factors vs character; read.quitte vs as.quitte(read.report())
data <- 
  read.quitte(params$mif, factors=FALSE) %>% 
  mutate(variable = str_remove(variable, "\\+{1,2}\\|")) # In the variable names, remove all occurrences of one or two consecutive "+" followed by "|".
histData <- 
  read.quitte(params$hist, factors=FALSE) %>% 
  mutate(variable = str_remove(variable, "\\+{1,2}\\|"))
mainReg <- params$mainReg
mainReg <- "World" # TODO: World vs GLO
y_bar <- params$y_bar
```
