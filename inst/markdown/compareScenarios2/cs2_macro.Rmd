...
```{r GDP-MER}
show_default_line_plots(data, "GDP|MER")
```
...
```{r GDP-PPP}
show_default_line_plots(data, "GDP|PPP")
```

## GDP - PPP per Capita

```{r GDP-PPP pCap}
show_default_line_plots(data, "GDP|PPP pCap")
```
...
```{r Capital Stock}
show_default_line_plots(data, "Capital Stock|Non-ESM")
```

## Investments

```{r Investments-Non-ESM}
show_default_line_plots(data, "Investments|Non-ESM")
```
...
```{r Interest Rate}
show_default_line_plots(data, "Interest Rate (t+1)/(t-1)|Real")
```
...
variables %>% 
  ...
  vars
walk(vars, show_default_line_plots, data=d)
```
...
vars %>% 
  str_subset("Liquids") %>% 
  walk(show_default_line_plots, data=d)
vars %>% 
  str_subset("Gases") %>% 
  walk(show_default_line_plots, data=d)
vars %>% 
...
variables %>% 
  ...
  vars
walk(vars, show_default_line_plots, data=data)
```
...
walk(kaya_vars, show_default_line_plots, data=kaya_rel)
```
