...
# stock_t0 <- readSource(type = 'Mueller', subtype = 'stocks', convert = FALSE) %>% 
#     madrat_mule() %>% 
#     filter('med' == estimate)

#extract names of available scenarios
#scn <- data

primary <- data %>% 
 
  #delete me!---
  filter(region %in% c('LAM')) %>%
  filter(scenario %in% c('SSP2EU-AMT-Base','SSP2EU-AMT-PkBudg670')) %>% 
  #-------------
  
  filter(variable %in% c('Production|Industry|Steel|Primary','Population')) %>% 
  calc_addVariable(
#   t                               #t  
   '`Primary steel accumulation`' = 'cumsum(`Production|Industry|Steel|Primary`)',
   units = 't'
  )  %>% 
  calc_addVariable(
#   t                                          #t                             #humans 10^6 #ADD FACTOR 1e6!!!!
   '`Primary steel accumulation per-capita`' = '`Primary steel accumulation` / `Population`',
   units = 't'
  )
# to do:
# integrate over interval duration
# add initial stocks

showLinePlots(primary, "Primary steel accumulation")
showLinePlots(primary, "Primary steel accumulation per-capita")
```
