...
if (all(items %in% data$variable)) {
  base_tax <- data[data$variable == "Price|Carbon" & data$scenario != "historical", "value"]
  buildings_tax <- data[data$variable == "Price|Carbon|Demand|Buildings", "value"]
  transport_tax <- data[data$variable == "Price|Carbon|Demand|Transport", "value"]
  # only create plots if a sector tax is unequal to the base co2 tax
  if (any(buildings_tax != base_tax) | any(transport_tax != base_tax)) {
