...
scen <- readGDX(gdx,"c_expname")[[1]]

vars <- c(
  readGDX(gdx, "ppf", restore_zeros = F),
  readGDX(gdx, "industry_ue_calibration_target_dyn37", restore_zeros = F)
)

# baseline run quantities
q.baseline <- readGDX(gdx, "vm_cesIO", restore_zeros = F)[, , "l"] %>% dimReduce()

# calibration data quantities
q.calibration <- readGDX(gdx, "pm_cesdata", restore_zeros = F)[, , "quantity"]  %>% dimReduce()
q.baseline <- q.baseline[, , vars]
q.calibration <- q.calibration[, , vars]

# baseline run prices
p.baseline <- readGDX(gdx, "o01_CESderivatives", restore_zeros = F)[, , "inco"] %>% dimReduce()

# calibration data prices
p.calibration <- readGDX(gdx, "pm_cesdata", restore_zeros = F)[, , "price"] %>% dimReduce()
...
for (r in regions) {
  ...
  data <- rbind(
    as.quitte(q.baseline[r, , ]) %>% select(region, variable = all_in, period, value) %>% mutate(model := scen),
    as.quitte(q.calibration[r, , ]) %>% select(region, variable = all_in, period, value) %>% mutate(model := "Calib. Target")
  )
  p <- ggplot() +
    geom_line(
      data = data,
  ...
  data <- rbind(
    as.quitte(p.baseline[r, , ]) %>% select(region, variable = all_in1, period, value) %>% mutate(model := scen),
    as.quitte(p.calibration[r, , ]) %>% select(region, variable = all_in, period, value) %>% mutate(model := "Calib. Target")
  )
  p <- ggplot() +
    geom_line(
      data = data,
